...
{   
    if (puzzle != NULL) {
        if ((x > -1 && x < puzzle->size) && (y > -1 && y < puzzle->size)) {
...
{   
    if (puzzle != NULL) {
        if ((x > -1 && x < puzzle->size) && (y > -1 && y < puzzle->size) && (value > 0 && value <= puzzle->size)) {
...
void update_adjacent_box_counters(puzzle_t* puzzle, int x, int y, int value) 
{
    ...
    for (int i = 0; i < puzzle->size; i++) { // for column of current box
    ...
    // if (x % sqrt_size == 1) { a = -1; }
    // else if (x % sqrt_size == 2) { a = -2; }
    // else { a = 0; }

    // if (y % sqrt_size == 1) { b = -1; }
    // else if (y % sqrt_size == 2) { b = -2; }
    // else { b = 0; }

    for (int m = a; m < sqrt_size + a; m ++) {
        for (int n = b ; n < sqrt_size + b; n++) { 
            if ((x + m != x) || (y + n != y)) {
...
void puzzle_print_formated (puzzle_t* puzzle, FILE* fp)
{
    ...
    // the (sqrt x sqrt) of size sudoku's in the first row
    double sqrt_size_pr = sqrt(puzzle->size);
    for ( int i = 0; i < sqrt_size_pr; i++){  //first three rows

        for(int j= 0; j < puzzle->size; j++){ // all their collumns
    ...
    for ( int i = sqrt_size_pr; i < (sqrt_size_pr*2); i++){
